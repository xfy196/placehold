...
router.get("/:size/:bgColor/:textColor/:ext", async (ctx) => {
    const {size, textColor, bgColor, ext = "png"} = ctx.params;
    const {text = ""} = ctx.query;
    let match = null;
    match = size.match(/^(\d+)$/);
    if (match) {
...
async function generatePlaceholdImg({
    ...
    const newText = is404 ? "4 0 4" : text ? text.replace("+", ' ') : width + " x " + height;
    const options = {
        fontSize,
        x: 0,
        y: 0,
